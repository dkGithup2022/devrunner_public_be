...
1. **개발 생산성 공유**
   Spring 개발에서 반복되는 부분을 Claude Code 스크립트에 넣어서 개발 과정을 압축하니까 피로도가 줄고 스피드가 빨라졌습니다. 이걸 공유하고 싶어서 공개합니다.

2. **코드 리뷰 및 피드백**
   지인들한테 소스를 보여주고 코드에 대해서 얘기해보고 싶어서 공개합니다.
   - 어디가 부족한지
   - 어디가 투머치인지
   - (1)로 자동화 가능한 부분이 있는지
...
### 스크립트 레포지토리
[클로드 코드 | 멀티모듈 프로젝트 생성 커맨드 모음](https://github.com/dkGithup2022/claude_code_multimodule_script)

위의 레포는 제가 각 멀티모듈에 도메인 패키지 단위로 소스를 추가하는 스크립트를 제작한 것입니다.
이걸 이용해서 이전에 운영하던 개발자 커뮤니티를 리뉴얼한 시행입니다.
...
완성된 아키텍처(헥사고날, 포트 앤 어댑터)보다는 **MVP와 실개발 모두에서 보편적으로 이득이 되는 중간 구조**를 채택했습니다.
